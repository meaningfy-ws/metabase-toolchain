...
if not METABASE_API_SSL_VERIFY:
    import urllib3
...
class MetabaseApi:
    ...
    def __init__(self, api_url, username, password, debug=False, ssl_verify: bool = None, cache_session: bool = True):
        if not api_url.endswith(self.BASE_URL):
            api_url += self.BASE_URL

        self.ssl_verify = ssl_verify if ssl_verify is not None else METABASE_API_SSL_VERIFY
    ...
    def query(self, method, query_name, json_data=None):
        ...
        query_url = self.api_url + query_name
    ...
    def create_session_if_needed(self):
        if self.cache_session and self.metabase_session:
            return
        self.create_session()
    ...
    def create_database(self, name, engine, details=None, is_full_sync=True, is_on_demand=False, auto_run_queries=True):
        self.create_session_if_needed()
        data = self.get_database(name, False, False)
        if data:
            return data
        return self.query('POST', 'database',
                          {"name": name, 'engine': engine, "details": details or {}, "is_full_sync": is_full_sync,
    ...
    def field_id2tablenameandfieldname(self, database_name, field_id):
        if not self.database_export.get(database_name):
            self.database_export[database_name] = self.get_database(database_name, True)
        if not field_id:
            return ['', '']
        db = self.database_export.get(database_name)
        if db:
    ...
    def card_id2name(self, database_name, card_id):
        if not self.cards_export.get(database_name):
            self.cards_export[database_name] = self.get_cards(database_name)
        db_cards = self.cards_export.get(database_name)
        if db_cards:
    ...
    def metric_id2name(self, database_name, metric_id):
        if not self.metrics_export.get(database_name):
            self.metrics_export[database_name] = self.get_metrics(database_name)
        db_metrics = self.metrics_export.get(database_name)
        if db_metrics:
    ...
    def update_fields(self, database_name, fields):
        output = []
        for f in fields:
            output.append(self.update_field(database_name, f))
        return output
    ...
    def dashboard_name2id(self, database_name, dashboard_name):
        if not self.dashboards_name2id.get(database_name):
            self.dashboards_name2id[database_name] = {}
            for d in self.get_dashboards(database_name):
                if self.dashboards_name2id[database_name].get(d['name']):
                    print("dashboard " + d['name'] + " not unique (already registered with id " + str(
                        self.dashboards_name2id[database_name].get(d['name'])) + " and trying to create it with id " + str(
                        d['id']) + ")")
                    continue
                self.dashboards_name2id[database_name][d['name']] = d['id']

        db_dashboards_name2id = self.dashboards_name2id.get(database_name)
        if db_dashboards_name2id:
            return db_dashboards_name2id.get(dashboard_name)
        else:
            return None
    ...
    def dashboard_id2name(self, database_name, dashboard_id):
        ...
        db_dashboards_name2id = self.dashboards_name2id.get(database_name)
        if db_dashboards_name2id:
            for dname in db_dashboards_name2id.keys():
                if db_dashboards_name2id[dname] == dashboard_id:
                    return dname
        return None

    def snippet_name2id(self, database_name, snippet_name):
        if not self.snippets_name2id.get(database_name):
            self.snippets_name2id[database_name] = {}
            for s in self.get_snippets(database_name):
                self.snippets_name2id[database_name][s['name']] = s['id']
        db_snippets_name2id = self.snippets_name2id.get(database_name)
        if db_snippets_name2id:
            return db_snippets_name2id.get(snippet_name)
        else:
            return None

    def card_name2id(self, database_name, card_name):
        if not self.cards_name2id.get(database_name):
            self.cards_name2id[database_name] = {}
            for c in self.get_cards(database_name):
                self.cards_name2id[database_name][c['name']] = c['id']
        db_cards_name2id = self.cards_name2id.get(database_name)
        if db_cards_name2id:
            return db_cards_name2id.get(card_name)
        else:
            return None
    ...
    def metric_name2id(self, database_name, metric_name):
        if not self.metrics_name2id.get(database_name):
            self.metrics_name2id[database_name] = {}
            for c in self.get_metrics(database_name):
                self.metrics_name2id[database_name][c['name']] = c['id']
        db_metrics_name2id = self.metrics_name2id.get(database_name)
        if db_metrics_name2id:
            return db_metrics_name2id.get(metric_name)
        else:
            return None
    ...
    def convert_pc_names2id(self, database_name, collection_name, field_name, pc_names):
        if pc_names[0] != '%':
            raise ValueError('Not a convertible value')
        sep = pc_names.find('%', 1)
        if sep == -1:
            raise ValueError('Not a convertible value')
        [new_k, names] = pc_names[1:sep], pc_names[sep + 1:]
        if new_k == 'JSONCONV':
        ...
        if field_name == 'collection_name' and collection_name:
        ...
        if field_name == 'dashboard_name':
            return [new_k, self.dashboard_name2id(database_name, names)]
        if field_name == 'pseudo_table_card_name':
            card_id = self.card_name2id(database_name, names)
            if not card_id:
                raise ValueError('card_name ' + names + ' not found')
            return [new_k, 'card__' + str(card_id)]
        re_split = names.split('|')

        re_split_len = len(re_split)
        if re_split_len == 3:
            metric_id = self.metric_name2id(database_name, re_split[2])
            if metric_id:
                return [new_k, metric_id]
            # CHECKME: raise ValueError('metric not found: ' + re_split[2])
        elif re_split_len == 2:
            field = self.field_tablenameandfieldname2field(database_name, re_split[0], re_split[1])
            if field:
                return [new_k, field['id']]
            # CHECKME: raise ValueError('field not found: ' + re_split[0] + '/' + re_split[1])
        elif re_split_len == 1:
            table_id = self.table_name2id(database_name, re_split[0])
            return [new_k, table_id]
        # CHECKME: raise ValueError('Unknown ' + str(field_name) + ' %' + str(new_k) + '% type')
        return ['', '']
    ...
    def convert_names2ids(self, database_name, collection_name, obj):
        ...
        if isinstance(obj, list):
            if len(obj) and obj[0] in ['field', 'metric']:
                if obj[1][0] == '%':
                    [k_name, value] = self.convert_pc_names2id(database_name, collection_name, None, obj[1])
                    obj_res[1] = value
            else:
            ...
            for k in obj.keys():
                if k[0] == '%':
                    try:
                        [new_k, value] = self.convert_pc_names2id(database_name, collection_name, None, k)
                        obj_res.pop(k)
                    ...
                    [new_k, value] = self.convert_pc_names2id(database_name, collection_name, k, obj[k])
                    obj_res.pop(k)
                    obj_res[new_k] = value
                else:
    ...
    def export_database_to_json(self, database_name, dirname):
        db = self.get_database(database_name)
        if db:
            with open(dirname + "/database_" + database_name + ".json", 'w', newline='') as jsonfile:
    ...
    def dashboard_import(self, database_name, dash_from_json):
        ...
        self.dashboards_name2id[database_name] = None
    ...
    def card_import(self, database_name, card_from_json):
        ...
        card_id = self.card_name2id(database_name, card_from_json['name'])
        if card_id:
            return self.query('PUT', 'card/' + str(card_id), card_from_json)
        self.cards_name2id[database_name] = {}
    ...
    def import_snippets_from_json(self, database_name, dirname, collection_name=None):
        res = []
        json_data = self.get_json_data('snippet_', dirname)
        if len(json_data):
            errors = None
    ...
    def import_cards_from_json(self, database_name, dirname, collection_name=None):
        ...
        if len(json_data):
            ...
            for card in json_data:
                try:
                    res.append(
                        self.card_import(database_name, self.convert_names2ids(database_name, collection_name, card)))
                except ValueError as e:
                    if not errors:
    ...
    @classmethod
    def import_files_from_dirname(cls, prefix, dirname):
        files = []
    ...
    def get_json_data(self, prefix, dirname):
        json_data = []
        for filename in self.import_files_from_dirname(prefix, dirname):
    ...
    def import_metrics_from_json(self, database_name, dirname):
        res = []
        json_data = self.get_json_data('metric_', dirname)
        if json_data:
            errors = None
    ...
    def import_dashboards_from_json(self, database_name, dirname, collection_name=None):
        res = [[], [], []]
        json_data = self.get_json_data('dashboard_', dirname)
        if len(json_data):
            json_data = self.convert_names2ids(database_name, collection_name, json_data)
            for dash in json_data:
